...
2. **Runtime mounting.** Discovers every CDI bean of type
   ...
   the user via `@Produces`) and mounts each operation as its own Vert.x
   route on Quarkus's main HTTP router. There is **no separate Smithy
   listener**: operations share the Quarkus HTTP server's port (per
   ADR-0003), and each operation is registered with its protocol-defined
   path (`@http(method, uri)` for restJson1; computed
   `POST /service/<Name>/operation/<Op>` for rpcv2). The bridging is done
   by the upstream
   [`:server:server-vertx-bridge`](../server/server-vertx-bridge/) module.
...
| `quarkus.smithy.server.shutdown-grace` | `10s`   | Bound on `BoundBridge.shutdown()` (best-effort).                       |
...
bridge inherits whatever the Quarkus HTTP server speaks.
...
- `runtime/` — runtime classpath, ships with the application. Contains
  ...
  `:server:server-api` and `:server:server-vertx-bridge`.
...
- `@streaming` blob operations are unsupported in this release; the
  bridge buffers request bodies via Vert.x's `BodyHandler`. Tracked as
  open question 6 in ADR-0006.
